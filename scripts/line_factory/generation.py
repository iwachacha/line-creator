...
def build_image_generation_plan(project: Project) -> str:
    ...
    lines = [
        ...
        "- Technical validation is not market readiness. Before production, create `reports/market_research_notes.md`, `reports/character_design_lock.md`, `reports/character_consistency_qa.md`, and `reports/item_communication_plan.md`.",
        "- Lock a memorable character first: silhouette, face, expression range, signature visual hook, identity anchors, and text system.",
        "- For repeated-character packs, create a model-sheet or reference-sheet contract before final generation. It must define immutable proportions, face layout, accessory positions, palette, and allowed squash/stretch limits.",
        ...
        "Character identity contract: use the approved model sheet/reference image; keep the same character, fixed proportions, face spacing, line weight, palette, signature accessories, and text system across every item; vary only pose, expression, prop, and camera distance as approved.",
        ...
        "- For repeated-character packs, compare every item against the approved model sheet before accepting it. Regenerate if body shape, face spacing, accessory positions, line weight, or palette drift enough that it reads as a different character.",
    ...
    for item in items:
        ...
        lines.extend(
            [
                ...
                text_part,
                "Character consistency: keep the approved recurring character identical to the model sheet: same silhouette, proportions, facial feature spacing, signature marks/accessories, palette, line weight, and text style. Change only the planned pose, emotion, prop, and wording for this item.",
...
def build_generation_qa_checklist(project: Project) -> str:
    ...
    lines = [
        ...
        "- [ ] `reports/character_consistency_qa.md` exists for repeated-character packs.",
        "- [ ] A model sheet or approved reference image was used for final production prompts.",
        "- [ ] Body silhouette, proportions, face spacing, signature accessories, palette, line weight, and text style remain the same character across the pack.",
        "- [ ] Only approved variables changed: pose, expression, prop, camera distance, and item wording.",
        "- [ ] Any item with character identity drift was regenerated before acceptance.",
        "",
        "## Text Integration",
