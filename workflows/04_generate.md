...
- Create `reports/character_consistency_qa.md` for repeated-character packs. Lock the approved model sheet/reference image, identity anchors, allowed variables, forbidden drift, and per-item consistency review fields.
...
- Include the character identity contract in every item prompt: same silhouette, proportions, face spacing, signature accessories, palette, line weight, and text style; change only the approved pose, expression, prop, camera distance, and wording.
...
- Record raw-output evidence, prompt references, source filenames, identity reference, character-consistency notes, approval notes, and known exceptions in `assets/working/source_manifest.yml`.
...
- Check exact text, typos, mojibake, duplicated tiny text, character identity drift, style drift, physical sticker artifacts, rights risk, expression variety, and transparent-background readiness.
- Regenerate any item whose face spacing, body proportions, signature accessory placement, outline weight, or palette makes it read as a different character.
